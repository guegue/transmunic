...
def inversion_categoria_chart(municipio=None, year=None, portada=False):
    ...
    if municipio:
        ...
        mi_clase_anios = list(ClasificacionMunicAno.objects.
        ...
        for row in otros:
            ...
            row['ejecutado_percent'] = percentage(row['ejecutado'], total_poblacion)
            row['asignado_percent'] = percentage(row['asignado'], total_poblacion)
        ...
        # obtiene datos para grafico comparativo de tipo de inversions
        tipo_inicial = list(Proyecto.objects.
        ...
        tipo_actualizado = list(Proyecto.objects.
                                filter(inversion__anio=year,
                                       inversion__periodo=PERIODO_ACTUALIZADO).
                                values('catinversion__nombre').
                                annotate(asignado=Sum('asignado')))

        tipo_final = list(Proyecto.objects.
                          filter(inversion__anio=year,
                                 inversion__periodo=PERIODO_FINAL).
                          values('catinversion__nombre').
                          order_by('catinversion__nombre').
        ...
        area_inicial = list(Proyecto.objects.
                            filter(inversion__anio=year,
                                   inversion__periodo=PERIODO_INICIAL).
                            values('areageografica').
                            order_by('areageografica').
                            annotate(asignado=Sum('asignado')))
        area_final = list(Proyecto.objects.
                          filter(inversion__anio=year,
                                 inversion__periodo=PERIODO_FINAL).
                          values('areageografica').
                          order_by('areageografica').
        ...
        fuente_inicial = list(InversionFuenteDetalle.objects.
                              filter(inversionfuente__anio=year,
                                     inversionfuente__periodo=PERIODO_INICIAL).
                              values('fuente__nombre').
                              order_by('fuente__nombre').
                              annotate(asignado=Sum('asignado')))

        fuente_final = list(InversionFuenteDetalle.objects.
                            filter(inversionfuente__anio=year,
                                   inversionfuente__periodo=PERIODO_FINAL).
                            values('fuente__nombre').
                            order_by('fuente__nombre').
                            annotate(ejecutado=Sum('ejecutado')))

        fuente = glue(fuente_inicial, fuente_final, 'fuente__nombre')
        fuente_actual = list(InversionFuenteDetalle.objects.
                             filter(inversionfuente__anio=year,
                                    inversionfuente__periodo=periodo).
                             values('fuente__nombre').
                             order_by('fuente__nombre').
                             annotate(ejecutado=Sum(quesumar)))
