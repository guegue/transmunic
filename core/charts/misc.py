...
def inversion_categoria_chart(request):
    ...
    for y in ys:
        ...
        if municipio and year:
            #FIXM wooot?
            value = IngresoDetalle.objects.filter(ingreso__year=year, subsubtipoingreso__origen__nombre=label, ingreso__municipio__clasificacionmunic=mi_clase).aggregate(total=Sum('asignado'))['total']
