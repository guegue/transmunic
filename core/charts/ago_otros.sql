SELECT muni.nombre, (
        (( SELECT SUM(core_ingresodetalle.{quesumar}) AS sum
          ...
          WHERE core_ingreso.anio = {year} AND core_Ingreso.periodo='{periodo}' AND core_tipoingreso.clasificacion = {tipoingreso} AND core_tipoingreso.codigo <> '{notipoingreso}' AND core_Ingreso.municipio_id=muni.id)
        -
	...
	/
        NULLIF(( SELECT SUM(core_ingresodetalle.{quesumar}) AS sum
           FROM core_ingresodetalle
             JOIN core_ingreso ON core_ingresodetalle.ingreso_id = core_ingreso.id
             JOIN core_tipoingreso ON core_ingresodetalle.tipoingreso_id = core_tipoingreso.codigo
          WHERE core_ingreso.anio = {year} AND core_Ingreso.periodo='{periodo}' AND core_tipoingreso.clasificacion = {tipoingreso} AND core_tipoingreso.codigo <> '{notipoingreso}' AND core_Ingreso.municipio_id=muni.id), 0)
