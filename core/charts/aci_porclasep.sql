SELECT clasificacion, (
        ...
        ( SELECT SUM(core_ingresodetalle.{quesumar}) AS sum
         ...
         core_Ingreso.anio=lugar_clasificacionmunicano.anio                 
          WHERE core_ingreso.anio = {year} AND core_Ingreso.periodo='{periodo}' AND core_tipoingreso.clasificacion = {tipoingreso} AND lugar_clasificacionmunicano.clasificacion_id=clase.id)
        - 
        ...
        NULLIF( (SELECT SUM(core_ingresodetalle.{quesumar}) AS sum
           FROM core_ingresodetalle
             ...
             JOIN lugar_clasificacionmunicano ON core_Ingreso.municipio_id=lugar_clasificacionmunicano.municipio_id AND
         core_Ingreso.anio=lugar_clasificacionmunicano.anio                 
          WHERE core_ingreso.anio = {year} AND core_Ingreso.periodo='{periodo}' AND core_tipoingreso.clasificacion = {tipoingreso} AND lugar_clasificacionmunicano.clasificacion_id=clase.id), 0)
    ) * 100 AS {quesumar}  FROM lugar_clasificacionmunic AS clase ORDER BY clasificacion;
