...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        rows = IngresoDetalle.objects.values('ingreso', 'codigo').annotate(
            id=Max('id'), count=Count('*')).filter(count__gt=1)
        for row in rows:
            deleted = IngresoDetalle.objects.filter(
                ingreso=row['ingreso'], codigo=row['codigo']).exclude(id=row['id']).delete()
            self.stdout.write(self.style.SUCCESS('Successfully deleted {} rows (ingreso={}, codigo={})'.format(
                deleted, row['ingreso'], row['codigo'])))
        rows = GastoDetalle.objects.values('gasto', 'codigo').annotate(
            id=Max('id'), count=Count('*')).filter(count__gt=1)
        for row in rows:
            deleted = GastoDetalle.objects.filter(
                gasto=row['gasto'], codigo=row['codigo']).exclude(id=row['id']).delete()
            self.stdout.write(self.style.SUCCESS(
                'Successfully deleted {} rows (gasto={}, codigo={})'.format(deleted, row['gasto'], row['codigo'])))
