SELECT ROW_NUMBER() OVER(ORDER BY fecha) AS id, periodo.fecha,
    (
        ...
        ( SELECT sum(core_ingresodetalle.ejecutado) AS sum
          ...
          WHERE core_ingreso.fecha = periodo.fecha AND core_tipoingreso.clasificacion = 0 AND lugar_municipio.slug = %s)
        - 
        ( SELECT sum(core_gastodetalle.ejecutado) AS sum
          ...
          WHERE core_gasto.fecha = periodo.fecha AND core_tipogasto.clasificacion = 0 AND lugar_municipio.slug = %s)
        )
        / 
        ( SELECT sum(core_ingresodetalle.ejecutado) AS sum
           FROM core_ingresodetalle
